...
class ReithStatus {
  ...
  func isConfigured() -> Bool {
    ...
    if let httpsProxy: String = store.getValueFromStoreDict(
      storeKey: proxiesKey,
    ...
    if configured {
      os_log("Network location is currently set to '%@'", log: OSLog.default, type: .debug,
  ...
  func isConnected() -> Bool {
    ...
    if let dnsDomainName: String = store.getValueFromStoreDict(
      storeKey: dnsKey,
      dictKey: Constants.DynamicStoreDictionaryKeys.reithDnsDomainNameKey
    ) {
      os_log("DNS Domain name is currently set to %@", log: OSLog.default, type: .debug,
             dnsDomainName)
