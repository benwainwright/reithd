...
public func startDaemon() {
  do {
    let dnsKey = SCDynamicStoreKeyCreateNetworkGlobalEntity(kCFAllocatorDefault,
...
func onDnsChange(store: SCDynamicStore, changed _: CFArray, info _: UnsafeMutableRawPointer?) {
  ...
  for configurer in reithConfigurers {
    configurer.reithStatus = reithStatus
    configurer.configureForReith()
  }
}

func initDynamicStoreMonitoringRunLoop(
  callback: @escaping SCDynamicStoreCallBack,
  keys: [CFString]? = nil,
  patterns: [CFString]? = nil
) throws {
  guard let store = SCDynamicStoreCreate(kCFAllocatorDefault, "reithd" as CFString, callback, nil)
  else {
