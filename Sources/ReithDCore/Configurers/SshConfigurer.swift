  ...
  private func doConfigure(enabled: Bool) {
    ...
    if fileManager.fileExists(atPath: sshFolder) {
      ...
      do {
        ...
        if enabled {
          configFileContents = unCommentLinesStartingWith(string: Constants.Strings.sshProxyCommand,
                                                          inContent: configFileContents)
        } else {
          configFileContents = commentLinesStartingWith(string: Constants.Strings.sshProxyCommand,
                                                        inContent: configFileContents)
        }
        os_log("Writing %@ back to disk", log: OSLog.default, type: .debug, configFile)
        try configFileContents
          .write(toFile: configFile, atomically: true, encoding: String.Encoding.utf8)
      } catch {
